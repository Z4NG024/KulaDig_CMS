...
class CheckCacheFolder {
  ...
  /* decodeFeedData: convert raw-json-data into array */
	private function checkCacheFolder() {
		...
		if ( !$wp_filesystem->is_dir( $this->cacheBaseFolder ) ) {			
			#$mkdirError = mkdir($this->cacheBaseFolder) ?? FALSE;
		   $mkdirError = $wp_filesystem->mkdir( $this->cacheBaseFolder );
        ...
        if (!is_dir($this->cacheFolder)) {
		  ...
		  $mkdirError = $wp_filesystem->mkdir( $this->cacheFolder );
          if (!$mkdirError) {
            # mkdir failed, usually due to missing write-permissions
            $this->errormsg .= "<hr><b>".__('caching not working, plugin aborted', 'json-content-importer').":</b><br>";
		...
		if ($wp_filesystem->is_writable( $this->cacheFolder )) {		
...
class FileLoadWithCache {
  ...
  /* get errorlevel */
	...
	private function storeFeedInCache(){
		...
		$this->cacheWritesuccess = $wp_filesystem->put_contents( $this->cacheFile, $this->feedData );
	...
	private function read_local_file_with_wp_filesystem($file_path) {
		if ( ! function_exists( 'request_filesystem_credentials' ) ) {
			require_once ABSPATH . 'wp-admin/includes/file.php';
		}
		$url = wp_nonce_url( 'index.php', 'my-nonce_jci_loadfile' );
		$credentials = request_filesystem_credentials( $url );
		if ( ! WP_Filesystem( $credentials ) ) {
			return '';#'Failed to initialize WP_Filesystem.';
		}
	...
	public function func_fallback2cache(){
		...
		if ($useCachedJson) {
			if(file_exists($this->cacheFile)) {
				...
				$this->feedData = $this->read_local_file_with_wp_filesystem($this->cacheFile);
