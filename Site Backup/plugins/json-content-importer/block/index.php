...
function checkCacheFolder($cacheBaseFolder, $cacheFolder) {
	# wp version 4.4.2 and later: "/cache" is not created at install, so the plugin has to check and create...
		...
		$url = wp_nonce_url( 'index.php', 'my-nonce-jci-cacher' );
		$credentials = request_filesystem_credentials( $url );
		if ( ! WP_Filesystem( $credentials ) ) {
            $this->errormsg .= __('Initialize of WP_Filesystem failed (checkCacheFolder)', 'json-content-importer');
	 ...
	 if (!$wp_filesystem->is_dir($cacheBaseFolder)) {
	   $mkdirError = $wp_filesystem->mkdir($cacheBaseFolder) ?? FALSE;
	   if (!$mkdirError) {
	 ...
	 if (!$wp_filesystem->is_dir($cacheFolder)) {
	   ...
	   $mkdirError = $wp_filesystem->mkdir($cacheFolder) ?? FALSE;
	   if (!$mkdirError) {
		 # mkdir failed, usually due to missing write-permissions
		 $errormsg .= "<hr><b>".__('caching not working, plugin aborted', 'json-content-importer').":</b><br>";
	...
	# if (!wp_is_writeable($cacheFolder)) {
 ...
 function jci_free_render( $attributes, $content ) {
	...
	if (1==$attributes['toggleswitch']) {
	 ...
	 if (1==$pluginOption_cacheStatus) {
		 ...
		 $cacheExpireTime = strtotime(gmdate('Y-m-d H:i:s'  , strtotime(" -".$pluginOption_cacheTime." " . $pluginOption_cacheTimeFormat )));
	...
	if (1==trim($attributes['toggleswitchjson'])) {
		$jsonout = wp_json_encode($jsonObj, JSON_PRETTY_PRINT);
		if (is_null($jsonObj)) {
			$shlen = 300;
			...
			$showjson = '<h4>JSON received from the API:</h4><pre style="background-color:#eee;">'.htmlentities(wp_json_encode($jsonObj, JSON_PRETTY_PRINT))."</pre>";
		}
	}
